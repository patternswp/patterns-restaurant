/*CSS*/
/* global PatternsRestaurantLocalize */
...
class PatternsRestaurantSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-restaurant-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-restaurant-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-restaurant-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-restaurant-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add(
						'patterns-restaurant-processing'
					);
					this.installPlugins();
				}
			}
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-restaurant-review-notice'
			);
